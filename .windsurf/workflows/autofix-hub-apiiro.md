---
description: Autopilot loop — keep producing Apiiro autofix PRs, respecting the 3-PR WIP cap, until the queue is drained
---

# /autofix-hub-apiiro

Run autofix-hub end-to-end for Apiiro on a continuous loop. Each iteration
...
1. **Refresh the queue.** Run `autofix-hub apiiro fetch --json`. Note the
   counts. If zero open issues, jump to step 4 (summarize) and exit.

2. **Loop.** Repeat until termination:

   a. `autofix-hub apiiro next-club --wait --poll-seconds 60 --max-wait-minutes 30`
      - This call:
   ...
   b. **Edit.** Operate exclusively inside `next_club.worktree_path`. Read
      ...
      `next_club.files`. Apply the fixes described in the prompts.
...
- Run `/autofix-hub-apiiro` again — `check-prs` reconciles any PRs that did get
