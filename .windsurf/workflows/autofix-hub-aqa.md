---
description: Autopilot loop — keep producing AQA accessibility autofix PRs, respecting the 3-PR WIP cap, until the queue is drained
---

# /autofix-hub-aqa

Run autofix-hub end-to-end for AQA (UsableNet accessibility) on a continuous
loop. Each iteration produces one PR. The CLI enforces the WIP cap (default 3
open `autofix`-labeled PRs); when the cap is reached, this workflow blocks and
polls until a PR is merged or closed before opening another.
...
1. **Refresh the queue.** Run `autofix-hub aqa fetch --json`.
   - For API-mode AQA, ensure `AQA_API_KEY`, `AQA_TEAM_SLUG`, and `AQA_TEST_ID`
     are set in `.env` (or pass `--api-key`, `--team-slug`, `--test-id`).
   - For Playwright-mode (axe/aqa engines), pass `--method playwright
     --engine axe --urls <comma-separated>`.
   - Note the counts. If zero open issues, jump to step 4 (summarize) and exit.

2. **Loop.** Repeat until termination:

   a. `autofix-hub aqa next-club --wait --poll-seconds 60 --max-wait-minutes 30`
      - This call:
   ...
   b. **Edit.** Operate exclusively inside `next_club.worktree_path`. Read
      ...
      `next_club.files`. Apply the WCAG fixes described in the prompts.
...
- Run `/autofix-hub-aqa` again — `check-prs` reconciles any PRs that did get
  opened, and the loop will pick up from the next available cluster.
